...
router.get('/', optionalAuth, async (req, res) => {
  const { page = '1', limit = '20', type, minPrice, maxPrice, search } = req.query as Record<string, string>;
...
router.patch('/:id', authenticate, validate(UpdateProductSchema), async (req, res) => {
  ...
  if (product.creatorId !== req.user!.id) { res.status(403).json({ error: 'Not your product' }); return; }
...
router.delete('/:id', authenticate, async (req, res) => {
  ...
  if (product.creatorId !== req.user!.id && req.user!.role !== 'ADMIN') {
    res.status(403).json({ error: 'Not your product' }); return;
  }
