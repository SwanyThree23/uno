...
router.get('/revenue', authenticate, async (req, res) => {
  ...
  // Group by day
  const byDay: Record<string, { date: string; revenue: number; gross: number; count: number }> = {};
  for (const tx of transactions) {
    const day = tx.createdAt.toISOString().slice(0, 10);
    if (!byDay[day]) byDay[day] = { date: day, revenue: 0, gross: 0, count: 0 };
    byDay[day].revenue += Number(tx.creatorAmount);
    byDay[day].gross   += Number(tx.grossAmount);
    byDay[day].count   += 1;
  }

  const total = transactions.reduce((sum: number, t: any) => sum + Number(t.creatorAmount), 0);
  ...
  res.json({
    ...
    byType: {
      superchats: transactions.filter((t: any) => t.type === 'SUPERCHAT').reduce((s: number, t: any) => s + Number(t.creatorAmount), 0),
      tips:       transactions.filter((t: any) => t.type === 'TIP').reduce((s: number, t: any) => s + Number(t.creatorAmount), 0),
      products:   transactions.filter((t: any) => t.type === 'PRODUCT_SALE').reduce((s: number, t: any) => s + Number(t.creatorAmount), 0),
    },
  });
});
