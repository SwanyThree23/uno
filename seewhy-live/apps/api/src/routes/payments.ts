...
router.post('/create-intent', authenticate, paymentLimiter, validate(PaymentSchema), async (req, res) => {
  const { toUserId, stageId, grossAmountCents, currency, type, message } = req.body as z.infer<typeof PaymentSchema>;
...
router.get('/history', authenticate, async (req, res) => {
  ...
  const { page = '1', limit = '20', direction = 'received' } = req.query as Record<string, string>;
