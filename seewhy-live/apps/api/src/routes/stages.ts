...
router.get('/:id', optionalAuth, async (req, res) => {
  ...
  if (!stage.roomId) {
    ...
    (stage as Record<string, unknown>).roomId = roomId;
  }
...
router.post('/', authenticate, requireCreator, validate(CreateStageSchema), async (req, res) => {
  const data = req.body as z.infer<typeof CreateStageSchema>;
...
router.patch('/:id', authenticate, validate(UpdateStageSchema), async (req, res) => {
  ...
  if (stage.creatorId !== req.user!.id && req.user!.role !== 'ADMIN') {
    res.status(403).json({ error: 'Not your stage' }); return;
  }

  const data = req.body as z.infer<typeof UpdateStageSchema>;
...
router.post('/:id/start', authenticate, streamLimiter, async (req, res) => {
  const stage = await prisma.stage.findUnique({ where: { id: req.params.id as string } });
  if (!stage) { res.status(404).json({ error: 'Stage not found' }); return; }
  if (stage.creatorId !== req.user!.id) { res.status(403).json({ error: 'Not your stage' }); return; }
  if (stage.status === 'LIVE') { res.status(409).json({ error: 'Stage already live' }); return; }
...
router.post('/:id/join', authenticate, async (req, res) => {
  const stage = await prisma.stage.findUnique({ where: { id: req.params.id as string } });
  if (!stage) { res.status(404).json({ error: 'Stage not found' }); return; }
  if (stage.status === 'ENDED') { res.status(409).json({ error: 'Stream has ended' }); return; }
  ...
  if (guestCount >= stage.guestLimit) {
    res.status(409).json({ error: 'Stage is full' }); return;
  }
