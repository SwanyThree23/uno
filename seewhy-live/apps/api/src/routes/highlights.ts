...
router.post('/generate', authenticate, validate(GenerateSchema), async (req: Request, res: Response) => {
  const { streamId, transcriptContext } = req.body as z.infer<typeof GenerateSchema>;
  ...
  const { compressedPrompt, savedTokens } = await compressWithLLMLingua(
    "Please generate highlights for the following stream segment: ", 
    transcriptContext
  );
