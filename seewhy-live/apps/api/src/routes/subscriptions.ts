...
const TIER_PRICES: Record<string, { amount: number; priceEnvKey: string }> = {
...
router.delete('/:creatorId/:tier', authenticate, async (req, res) => {
  ...
  const sub = await prisma.subscription.findUnique({
    where: { subscriberId_creatorId_tier: { subscriberId: req.user!.id, creatorId, tier: tier as 'BRONZE' | 'SILVER' | 'GOLD' } },
  });
...
router.post('/webhook', async (req, res) => {
  ...
  try {
    event = constructWebhookEvent(req.body as Buffer, sig, process.env.STRIPE_WEBHOOK_SECRET || '');
  } catch {
    res.status(400).json({ error: 'Webhook signature invalid' }); return;
  }
